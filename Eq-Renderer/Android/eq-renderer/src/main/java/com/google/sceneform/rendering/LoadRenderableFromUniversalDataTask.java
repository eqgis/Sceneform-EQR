...
public class LoadRenderableFromUniversalDataTask<T extends Renderable> {
  ...
  private final RenderableInternalPlyData renderableData;
  ...
  LoadRenderableFromUniversalDataTask(
    ...
    if (data instanceof RenderableInternalPlyData) {
      this.renderableData =
          (RenderableInternalPlyData) data;
    } else {
  ...
  public CompletableFuture<T> downloadAndProcessRenderable(
    ...
    return CompletableFuture.supplyAsync(
        ...
        .thenApplyAsync(
            (byte[] bytes) -> {
                this.renderableData.byteBuffer = bytes;
                return renderable;
            },
