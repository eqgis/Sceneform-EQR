...
本仓库包含多个分支，不同分支采用不同协议：

- **使用或依赖 eq-slam 的分支**
   ORB-SLAM3 本身为 **GPLv3** 许可，属于强 Copyleft 协议，eq-slam库引入了ORB-SLAM3。
   因此所有 **包含 eq-slam、修改 eq-slam、或通过 JNI 与 eq-slam 直接链接的代码** 的分支
   **必须使用 GPLv3 协议发布**。

- **不包含 eq-slam 的分支**
   main分支默认采用 **Apache License 2.0**，不受 GPL 的约束。

### 为什么要分支区分许可证？

由于 GPLv3 要求衍生作品必须以 GPLv3 发布，为避免整个仓库被 GPL“传染”，
 我们将所有 GPL 相关代码放入独立分支，其它未涉及 GPL 的分支继续保持 Apache 协议。

### 注意事项

- 如果基于 GPL 分支创建新分支，该分支也必须继续使用 **GPLv3**。
- 如果完全移除 GPL 代码，可以将该分支切换回 **Apache 2.0**。
- 协议的区分基于 **分支**，而非整个仓库。

完整许可证参见每个分支的LICENSE部分：

- `LICENSE-GPLv3`（GPL 分支）
- `LICENSE`（Apache 2.0 分支）
